...
class SuperTokensInterceptorWrapper extends Interceptor {
  ...
  void onRequest(
    ...
    logDebugMessage(
        'SuperTokensInterceptorWrapper.onRequest: Intercepting request call');
    ...
    if (!shouldRunDioInterceptor(options)) {
      logDebugMessage(
          'SuperTokensInterceptorWrapper.onRequest: Skipping dio interceptor');
      return super.onRequest(options, handler);
    }

    logDebugMessage(
        'SuperTokensInterceptorWrapper.onRequest: Running dio interceptor');
    if (Client.cookieStore == null) {
      logDebugMessage(
          'SuperTokensInterceptorWrapper.onRequest: Initializing cookie store');
      Client.cookieStore = SuperTokensCookieStore();
    }

    logDebugMessage(
        'SuperTokensInterceptorWrapper.onRequest: Removing auth header if it matches local token');
    options = await _removeAuthHeaderIfMatchesLocalToken(options);

    logDebugMessage(
        'SuperTokensInterceptorWrapper.onRequest: Getting local session state');
    ...
    if (antiCSRFToken != null) {
      logDebugMessage(
          'SuperTokensInterceptorWrapper.onRequest: antiCSRFToken is not null');
    ...
    logDebugMessage(
        'SuperTokensInterceptorWrapper.onRequest: Setting authorization header if required');
    ...
    logDebugMessage(
        'SuperTokensInterceptorWrapper.onRequest: uriForCookieString: ${uriForCookieString}');
    ...
    if (existingCookieHeader != null) {
      logDebugMessage(
          'SuperTokensInterceptorWrapper.onRequest: Combining cookie header values');
      options.headers[HttpHeaders.cookieHeader] =
          "$existingCookieHeader;${newCookiesToAdd ?? ""}";
    } else {
      logDebugMessage(
          'SuperTokensInterceptorWrapper.onRequest: Adding new cookie header');
      options.headers[HttpHeaders.cookieHeader] = newCookiesToAdd ?? "";
    }

    logDebugMessage(
        'SuperTokensInterceptorWrapper.onRequest: Injecting status check in validateStatus');
    ...
    logDebugMessage(
        'SuperTokensInterceptorWrapper.onRequest: Calling next on handler');
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) async {
    logDebugMessage(
        'SuperTokensInterceptorWrapper.onResponse: Intercepting response call');
    if (!shouldRunDioInterceptor(response.requestOptions)) {
      logDebugMessage(
          'SuperTokensInterceptorWrapper.onResponse: Skipping dio interceptor');
      return handler.next(response);
    }

    logDebugMessage(
        'SuperTokensInterceptorWrapper.onResponse: Running dio interceptor');
    _refreshAPILock.acquireWrite();
    logDebugMessage('SuperTokensInterceptorWrapper.onResponse: write acquired');
    logDebugMessage(
        'SuperTokensInterceptorWrapper.onResponse: saving tokens from headers');
    await saveTokensFromHeaders(response);
    String? frontTokenFromResponse =
        response.headers.map[frontTokenHeaderKey]?.first.toString();
    logDebugMessage(
        'SuperTokensInterceptorWrapper.onResponse: frontTokenFromResponse: ${frontTokenFromResponse}');
    ...
    logDebugMessage(
        'SuperTokensInterceptorWrapper.onResponse: setCookieFromResponse length: ${setCookieFromResponse?.length}');
    ...
    try {
      if (response.statusCode == SuperTokens.sessionExpiryStatusCode) {
         ...
         */
        logDebugMessage(
            'SuperTokensInterceptorWrapper.onResponse: Got expiry status code');
        ...
        if (sessionRefreshAttempts >=
          ...
          logDebugMessage(
              'SuperTokensInterceptorWrapper.onResponse: Max attempts of ${SuperTokens.config.maxRetryAttemptsForSessionRefresh} reached for refreshing, cannot continue');
          handler.reject(
            DioException(
        ...
        logDebugMessage(
            'SuperTokensInterceptorWrapper.onResponse: removing auth header if it matches token');
        requestOptions =
        ...
        if (shouldRetry.status == UnauthorisedStatus.RETRY) {
          logDebugMessage(
              'SuperTokensInterceptorWrapper.onResponse: Got RETRY status');
          logDebugMessage(
              'SuperTokensInterceptorWrapper.onResponse: Refreshing attempt: ${sessionRefreshAttempts + 1}');
          ...
          logDebugMessage(
              'SuperTokensInterceptorWrapper.onResponse: Calling next on handler');
          return handler.next(res);
        } else {
          if (shouldRetry.exception != null) {
            logDebugMessage(
                'SuperTokensInterceptorWrapper.onResponse: Got non null exception');
            handler.reject(
              DioException(
                  requestOptions: response.requestOptions,
  ...
  Future<void> saveTokensFromHeaders(Response response) async {
    logDebugMessage(
        'SuperTokensInterceptorWrapper.saveTokensFromHeaders: Saving tokens from header');
    String? frontTokenFromResponse =
        response.headers.map[frontTokenHeaderKey]?.first.toString();
    logDebugMessage(
        'SuperTokensInterceptorWrapper.saveTokensFromHeaders: frontTokenFromResponse: ${frontTokenFromResponse}');

    if (frontTokenFromResponse != null) {
      logDebugMessage(
          'SuperTokensInterceptorWrapper.saveTokensFromHeaders: Setting token since it is not null');
    ...
    if (antiCSRFFromResponse != null) {
      ...
      logDebugMessage(
          'SuperTokensInterceptorWrapper.saveTokensFromHeaders: Setting token in AntiCSRF');
    ...
    logDebugMessage(
        'SuperTokensInterceptorWrapper.saveTokensFromHeaders: accessHeader: ${accessHeader}');
    if (accessHeader != null) {
      logDebugMessage(
          'SuperTokensInterceptorWrapper.saveTokensFromHeaders: Setting access header');
    ...
    logDebugMessage(
        'SuperTokensInterceptorWrapper.saveTokensFromHeaders: refreshHeader: ${refreshHeader}');
    if (refreshHeader != null) {
      logDebugMessage(
          'SuperTokensInterceptorWrapper.saveTokensFromHeaders: Setting refresh header');
  ...
  Future<RequestOptions> _removeAuthHeaderIfMatchesLocalToken(
    ...
    logDebugMessage(
        'SuperTokensInterceptorWrapper._removeAuthHeaderIfMatchesLocalToken: Removing auth header if it matches local token');
    if (req.headers.containsKey("Authorization") ||
        req.headers.containsKey("authorization")) {
      String? authValue = req.headers['Authorization'];
      logDebugMessage(
          'SuperTokensInterceptorWrapper._removeAuthHeaderIfMatchesLocalToken: authValue: ${authValue}');
      if (authValue == null) {
        logDebugMessage(
            'SuperTokensInterceptorWrapper._removeAuthHeaderIfMatchesLocalToken: Setting auth header');
      ...
      logDebugMessage(
          'SuperTokensInterceptorWrapper._removeAuthHeaderIfMatchesLocalToken: accessToken: ${accessToken}');
      logDebugMessage(
          'SuperTokensInterceptorWrapper._removeAuthHeaderIfMatchesLocalToken: refreshToken: ${refreshToken}');

      if (accessToken != null &&
          refreshToken != null &&
          authValue != "Bearer $accessToken") {
        logDebugMessage(
            'SuperTokensInterceptorWrapper._removeAuthHeaderIfMatchesLocalToken: Removing authorization headers');
  ...
  static Future<RequestOptions> _setAuthorizationHeaderIfRequired(
    ...
    logDebugMessage(
        'SuperTokensInterceptorWrapper._setAuthorizationHeaderIfRequired: Setting authorization header if required');
    String? accessToken = await Utils.getTokenForHeaderAuth(TokenType.ACCESS);
    String? refreshToken = await Utils.getTokenForHeaderAuth(TokenType.REFRESH);
    logDebugMessage(
        'SuperTokensInterceptorWrapper._setAuthorizationHeaderIfRequired: accessToken: ${accessToken}');
    logDebugMessage(
        'SuperTokensInterceptorWrapper._setAuthorizationHeaderIfRequired: refreshToken: ${refreshToken}');
    ...
    if (accessToken != null && refreshToken != null) {
      if (options.headers["Authorization"] != null ||
        ...
        //  no-op
        logDebugMessage(
            'SuperTokensInterceptorWrapper._setAuthorizationHeaderIfRequired: Doing nothing as headers are already set');
      } else {
        String tokenToAdd = addRefreshToken ? refreshToken : accessToken;
        logDebugMessage(
            'SuperTokensInterceptorWrapper._setAuthorizationHeaderIfRequired: Setting header to bearer: ${tokenToAdd}');
