...
class NormalisedURLDomain {
  ...
  static String normaliseUrlDomainOrThrowError(String input,
    ...
    logDebugMessage(
        'NormalisedURLDomain.normaliseUrlDomainOrThrowError: Normalising url domain: ${input}');
    try {
      if (!trimmedInput.startsWith("http://") &&
          !trimmedInput.startsWith("https://")) {
        logDebugMessage(
            'NormalisedURLDomain.normaliseUrlDomainOrThrowError: Does not start with http');
      ...
      logDebugMessage(
          'NormalisedURLDomain.normaliseUrlDomainOrThrowError: hostName: ${hostName}');
      logDebugMessage(
          'NormalisedURLDomain.normaliseUrlDomainOrThrowError: scheme: ${scheme}');
      logDebugMessage(
          'NormalisedURLDomain.normaliseUrlDomainOrThrowError: hasNoPort: ${hasNoPort}');
      logDebugMessage(
          'NormalisedURLDomain.normaliseUrlDomainOrThrowError: hostSuffix: ${hostSuffix}');

      if (ignoreProtocal) {
        logDebugMessage(
            'NormalisedURLDomain.normaliseUrlDomainOrThrowError: Ignoring protocol');
        ...
        logDebugMessage(
            'NormalisedURLDomain.normaliseUrlDomainOrThrowError: Keeping protocol');
        trimmedInput = scheme + "://" + hostSuffix;
      }

      logDebugMessage(
          'NormalisedURLDomain.normaliseUrlDomainOrThrowError: Normalised value: ${trimmedInput}');
      return trimmedInput;
    } catch (e) {}
